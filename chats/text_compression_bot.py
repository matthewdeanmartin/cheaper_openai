...
async def shorten_this_text(text:str,
                            target_model:str,
                            goal_tokens:int, kind_of_text:str)->str:
    current_token_count = count_tokens(text)
    if current_token_count <= goal_tokens:
        return text
    scores = readability_scores(text)
    scores_text = f"Current text's readability scores: Flesch-Kincaid: {scores['flesch_kincaid']}, " \
          f"Coleman-Liau: {scores['coleman_liau']}, Gunning Fog: {scores['gunning_fog']}. "

    instructions = """You help summarize and shorten texts."""
    short_bot = await get_persistent_bot("Text summarization and shortening bot",
                                         instructions)
...
if __name__ == "__main__":
    # Python 3.7+
    asyncio.run(shorten_this_text(LOTS_OF_TEXT,
                                  "uh....",
                                  700,
                                  "txt"
                                  ))
